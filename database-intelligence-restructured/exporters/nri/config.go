...
type RateLimitConfig struct {
	...
	// Global maximum RPS (across all databases)
	GlobalMaxRPS float64 `mapstructure:"global_max_rps"`
	
	// Global maximum burst
	GlobalMaxBurst int `mapstructure:"global_max_burst"`
	
	// Enable adaptive rate limiting
	EnableAdaptive bool `mapstructure:"enable_adaptive"`
	
	// Minimum RPS for adaptive limiting
	MinRPS float64 `mapstructure:"min_rps"`
	
	// Maximum RPS for adaptive limiting
	MaxRPS float64 `mapstructure:"max_rps"`
	
	// Per-database rate limits
	DatabaseLimits map[string]DatabaseRateLimit `mapstructure:"database_limits"`
}

// DatabaseRateLimit defines rate limit for specific database
type DatabaseRateLimit struct {
	RPS   float64 `mapstructure:"rps"`
	Burst int     `mapstructure:"burst"`
}
...
func (cfg *Config) Validate() error {
	...
	if cfg.RateLimiting.Enabled {
		if cfg.RateLimiting.RPS <= 0 {
			cfg.RateLimiting.RPS = 500 // Default 500 RPS
		}
		if cfg.RateLimiting.Burst <= 0 {
			cfg.RateLimiting.Burst = 100 // Default burst of 100
		}
		if cfg.RateLimiting.EnableAdaptive {
			if cfg.RateLimiting.MinRPS <= 0 {
				cfg.RateLimiting.MinRPS = 100
			}
			if cfg.RateLimiting.MaxRPS <= 0 {
				cfg.RateLimiting.MaxRPS = 2000
			}
		}
	}
