...
func TestRealQueryPatterns(t *testing.T) {
	...
	t.Run("Analytics_Workload", func(t *testing.T) {
		...
		queries := []string{
			`SELECT DATE(order_date), COUNT(*) FROM e2e_test.orders GROUP BY DATE(order_date)`,
...
func TestDatabaseErrorScenarios(t *testing.T) {
	...
	t.Run("Connection_Errors", func(t *testing.T) {
		...
		db, err := sql.Open("postgres", "host=localhost port=5433 user=postgres password=wrong dbname=e2e_test sslmode=disable")
		assert.NoError(t, err) // sql.Open doesn't connect
		defer db.Close()
		
		// The actual connection error happens on Ping
		err = db.Ping()
		assert.Error(t, err)
	})
