    ...
    for {
        ...
        for _, key := range keys {
            ...
            if v, ok := data["mean_duration"]; ok {
                stats.MeanDuration, _ = strconv.ParseFloat(v, 64)
            }
            if v, ok := data["error_rate"]; ok {
                stats.ErrorRate, _ = strconv.ParseFloat(v, 64)
            }
            if v, ok := data["importance_score"]; ok {
                stats.ImportanceScore, _ = strconv.ParseFloat(v, 64)
            }
            
            aa.queryStats[queryID] = stats
            
            if v, ok := data["sample_rate"]; ok {
                aa.sampleRates[queryID], _ = strconv.ParseFloat(v, 64)
            }
        }
