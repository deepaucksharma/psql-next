...
func (md *MySQLDetector) detectPerformanceSchema(ctx context.Context, features *FeatureSet) error {
	...
	perfSchemaTables := map[string]string{
		CapPerfSchemaStatementsDigest: "events_statements_summary_by_digest",
		CapPerfSchemaWaits:           "events_waits_summary_global_by_event_name",
		CapPerfSchemaStages:          "events_stages_summary_global_by_event_name",
	}
	
	for capability, table := range perfSchemaTables {
